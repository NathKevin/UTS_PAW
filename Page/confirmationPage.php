...
if (!$_SESSION['isLogin']) {
    ...
    if(isset($_POST['order'])){
        $qty=$_POST['qty'];
        $hargaTotal=$qty*$prod['harga'];
        if($qty>=1){
            $orderQuery="INSERT INTO cart SET user_id='$id', produk_id='$produkId', qty='$qty', harga_total='$hargaTotal', checkout=0";
           
            $order=mysqli_query($con,$orderQuery);
            echo $order;
            if($order==true){
                header("Location: ./shoppingOnline.php");
            }
        }else{
            $error1=true;
        }
...
<body>
    <div class="box">
        <div class="box2">
            <br />
            <img class="img" src="<?=$prod['gambar']?>" />
            <p class="tulisan" style="margin-top:20px;"><?=$prod['nama']?></p>
            <p class="tulisan" style="margin-top:-20px;">Rp. <?=$prod['harga']?></p>
            <p class="tulisan" style="margin-top:-20px;">Berat: <?=$prod['berat']?></p>
            <form method="POST">
                <div class="form-group">
                    <input type="number" placeholder="Quantity" name="qty" id="qty" />
                    <?php
                            if(isset($error1)){
                                echo "<p style='color:red;' class='salah'>". "Quantity tidak boleh kurang dari 1" . "</p>";
                            }
                        ?>
                    <button class="btn" type="submit" id="order" name="order">Order</button>
                    <a class="btn" type="button" href="./shoppingOnline.php">Cancel</a>
                </div>
                <br />
            </form>
